...
def chat_completion_kakaocorp_private(model, conv, temperature, max_tokens):
    client = openai.OpenAI(
        api_key="EMPTY",
        base_url="http://34.61.236.68:8000/v1",
    )
    ...
    for _ in range(API_MAX_RETRY):
        try:
            ...
            response = client.chat.completions.create(
                model="kanana-1.5-32.5b-instruct-2504",
                messages=messages,
                n=1,
...
def chat_completion_wandb(model, conv, temperature, max_tokens):
