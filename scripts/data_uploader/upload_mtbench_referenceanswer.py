...
with wandb.init(entity=args.entity, project=args.project, job_type="upload_data") as run:
    dataset_artifact = wandb.Artifact(name="mtbench_ko_referenceanswer", 
                                    type="dataset", 
                                    metadata={"version":args.dataset_version},
                                    description="This dataset is based on version {}".format(args.dataset_version))
    
    # track lineage
    run.use_artifact('wandb-japan/llm-leaderboard/mtbench_ja_prompt:v0', type='dataset') #'TBU'
    run.use_artifact('wandb-japan/llm-leaderboard/mtbench_ja_question:v0', type='dataset') #'TBU'
