...
def evaluate_robustness(subset: str, df: pd.DataFrame):
    ...
    # add symbol and incorrect to normal_df
    ...
    # Extract data for each task categorized as normal, symbol, and incorrect, and append the results to `normal_df`.
    for task in normal_df["task"].unique():
        ...
        # Extract one row each of normal, symbol, and incorrect for the same task and add the output to `normal_df`.
